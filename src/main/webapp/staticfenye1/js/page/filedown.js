/**
 * Created by zhangqiang  on 2016/11/2.
 */
...
$(function () {
   ...
   /* var sitesTable1 = $('#sites-table1').DataTable({
    ...
    var sitesTable = $('#sites-table').DataTable({
        ...
        processing: true,
        bStateSave:true,
        ajax: "selectFileManageByFileManageName.do",
        dataSrc: "",
        select: {
            style: "single",
            selector: "td:not(':last-child')"
                    ...
                    "render" : function (data, type, row ) {
                    	 if(data !=null){
                             if(row.xzl !=null){
                            	 var aa=row.idc;
                            	 //alert(aa);
                            	 /*/fileproscenium/down.do?"+ "id=" +  rowData.id  "++"
*/                                 return "<a href='/fileproscenium/down.do?id="+aa+"'>"+row.xzl+"</a>";
                             }
                         }else{
                             return "";
                         }
           ...
           /* { "data": "xzl" },*//*,
            ...
            /*{
                ...
                "render" : function (data, type, row ) {
                    var addDom = '<button class="btn btn-flat btn-sm bg-navy device-add-btn">添加设备</button>';
                    var editDom = '<button class="btn btn-flat btn-sm btn-success site-edit-btn">下载</button>';
                    var delDom = '<button class="btn btn-flat btn-sm btn-danger site-del-btn">删除</button>';
                    return  editDom;
                }
            }*/
...
/*
      ...
      //  alert(rowData.imsi);
           ...
           /* $('#site-dlg1').modal('show');*/
            var queryUrl = "/fileproscenium/down.do?"+ "id=" +  rowData.id ;
            //alert(queryUrl);
          sitesTable.ajax.url( queryUrl );
